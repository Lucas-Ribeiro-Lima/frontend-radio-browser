...
export function StationSearchWrapper({ children, isOpen }: Readonly<StationSearchWrapperProps>) {
  return(
    <aside className={`
        ...
        background-default
        lg:relative lg:search-open lg:w-1/3 lg:shadow-sm lg:shadow-slate-600
...
export function StationSearchContent({ children, toggleSearch, filterSearch }: Readonly<StationSearchContentProps>) {
  ...
  return(
    <div className='flex flex-col flex-1 '>
      ...
      <Collapsible className="flex flex-col flex-shrink items-center space-x-1 space-y-2">
        <Input
...
export function StationListWrapper({ children }:  Readonly<StationListWrapperProps>) {
  return(
    <ol className="flex-1 space-y-2 rounded-lg p-2 overflow-hidden">
      {children}
    </ol>
  )
}
...
export function StationPagination({ filterSearch }: Readonly<StationPaginationProps>) {
  ...
  return(
    <Pagination className="relative bot-0">
      <PaginationContent>
          <PaginationItem >
